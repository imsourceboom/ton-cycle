...
(async () => {
  ...
  const electionStatusH1 = document.querySelector('article#election-status h1');
  ...
  if (
    currentUnixTimestamp > latestElectionStart &&
    currentUnixTimestamp < latestElectionEnd
  ) {
    electionStatusH1.textContent = 'OPEN';
    electionStatusH1.style.cssText = `color: #21C49C`;
  } else {
    electionStatusH1.textContent = 'CLOSE';
    electionStatusH1.style.cssText = `color: #5B67A2`;
  }
  currentElecStartH1.textContent = dateConvertor(latestElectionStart);
  currentElecEndH1.textContent = dateConvertor(latestElectionEnd);
  nextElecStartH1.textContent = dateConvertor(nextElectionStart);
  nextElecEndH1.textContent = dateConvertor(nextElectionEnd);
})();
